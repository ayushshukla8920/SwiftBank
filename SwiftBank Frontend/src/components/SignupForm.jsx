...
const SignupForm = () => {
  ...
  const handleSubmit = async(e)=>{
    ...
    try {
      const response = await axios.post(`${serverURL}/signup`, { username, password, name, dob, mobno, email, gender }, { withCredentials: true });
  ...
  const handleOTPSubmit = async(event)=>{
    ...
    const dob = date.getFullYear()+'/'+(date.getMonth()+1)+'/'+date.getDate();
    try {
      const response = await axios.post(`${serverURL}/verify-otp`, { username, password, name, dob, mobno, email, gender, otp }, { withCredentials: true });
